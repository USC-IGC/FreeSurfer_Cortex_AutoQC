...
for subj_id in ${SUBJECTS[@]}; do 
    ...
    for hemi in lh rh; do
        ...
        printf ",%g" `awk 'NR > 61 {print $5}' ${stats_file}` >> ${outcsv}
        printf ",%g" `awk 'NR > 61 {print $3}' ${stats_file}` >> ${outcsv}
        printf ",%g" `awk 'NR > 61 {print $4}' ${stats_file}` >> ${outcsv}
        printf ",%g" `awk 'NR > 61 {print $6}' ${stats_file}` >> ${outcsv}
        printf ",%g" `awk 'NR > 61 {print $2}' ${stats_file}` >> ${outcsv}
        printf ",%g" `awk 'NR > 61 {print $7}' ${stats_file}` >> ${outcsv}
        printf ",%g" `awk 'NR > 61 {print $8}' ${stats_file}` >> ${outcsv}
        printf ",%g" `awk 'NR > 61 {print $9}' ${stats_file}` >> ${outcsv}
        printf ",%g" `awk 'NR > 61 {print $10}' ${stats_file}` >> ${outcsv}
    done
